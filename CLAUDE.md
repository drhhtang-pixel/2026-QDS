...
- **老師要求（2026/09/26）：每次修改完，一定要先把改好的 HTML 傳給老師看（SendUserFile，display: render），
  等老師確認後才繼續下一步**（例如合併到 main、上線、再做其他修改）。整頁網站版傳 `weekNN/index.html`；
  只改單份講義時可同時附上 `sources/weekNN/…html`。
- 讀者：研究所學生，會用電腦與手機瀏覽。
